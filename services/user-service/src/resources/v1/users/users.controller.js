...
// eslint-disable-next-line global-require
const getRedisConfig = () => require('../../../config/v1/redis');
// eslint-disable-next-line global-require
...
const filePathToURL = (req, filePath) => {
  ...
  const serviceName = process.env.USER_SERVICE_NAME;
  const fileUrl = `${baseUrl}/${serviceName}/${normalizedPath}`;
  return fileUrl;
};
...
const deleteLocalFile = (fileUrl) => {
  try {
    ...
    const servicePrefix = `/${process.env.USER_SERVICE_NAME}`;
    const relativePath = filePath.startsWith(servicePrefix)
      ? `.${filePath.replace(servicePrefix, '')}`
      : `.${filePath}`; // Go one directory back becasue uploads folder is outside the src folder
...
const getUserProfile = async (req, res) => {
  try {
    ...
    /**
     * To test redis pub/sub functionality
     * Publish test event to Redis (Gateway will pick this up)
    ...
    try {
      ...
      const eventMessage = JSON.stringify({
        userId: user.id,
        type: socketEvents.EMIT.USER_PROFILE_VIEWED,
        data: { message: 'Your profile was viewed', timestamp: new Date() },
      });
      await redisConfig.redisClient.publish(socketEvents.CHANNELS.SYSTEM_EVENTS, eventMessage);
      console.log(`Published event: ${socketEvents.EMIT.USER_PROFILE_VIEWED} for user ${user.id}`);
    } catch (err) {
